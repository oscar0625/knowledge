<template>
  <div class="discovery-project-list-page" v-if="info">
    <my-header bgColor="#f7f7f7">项目详情</my-header>
    <div class="synthesis">
      <div class="title">{{ info.name }}</div>
      <p class="time">
        <span>开始时间：{{ info.startDate | dateFilterSpace }}</span>
        <span>结束时间：{{ info.endDate | dateFilterSpace }}</span>
      </p>
      <p class="source">
        <i>发文来源：</i>
    ...
    <project-declare
      ...
      :showApply="info.itemDateState === 2"
    />
  </div>
</template>

<script>
import ProjectDeclare from "@/common/discover/project_declare";
...
export default {
  ...
  methods: {
    ...
    loadInfo() {
      ...
      Promise.all([
        this.$api("policy", "getProjectInfo", {
          id,
          type: 1
        }),
        this.$api("policy", "getProjectInfoRec", {
          id
        })
      ]).then(
