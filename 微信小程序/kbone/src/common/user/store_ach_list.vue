<template>
  <div class="store-ach-list">
    <ul>
      <li
        v-for="item in dataList"
        :key="item.id"
        :class="{
          'failure-data': item.status === 2
        }"
      >
        <a
          :href="`/${
            item.imTypeName === '产品类成果' ? 'prodAchInfo' : 'techAchInfo'
          }?id=${item.achiId}`"
          :target="isBlank"
        >
          <div class="title">
            <h4 class="fn-text-ellipsis">
              <span
                :class="{
                  tag: true,
                  prod: item.imTypeName === '产品类成果'
                }"
              >
                {{ item.imTypeName.slice(0, 2) + "成果" }}</span
              >{{ item.name }}
            </h4>
          </div>
          <div class="detail">
            <div
              ...
              background-image: url(${
                createFieldImg(item.imSpecialFieldIdName).a
              });
            `"
            >
              {{ item.imSpecialFieldIdName }}
            </div>
            <div
              class="score"
              v-if="item.totalScore && item.imTypeName === '技术类成果'"
            >
              <span
