<template>
  <div v-if="visible">
    <wx-animation ref="createProject" class="create-project-dialog">
      <div class="mask" @click="hideContent"></div>
      <mp-toptips
        type="error"
        :show="!!errorMessage"
        :msg="errorMessage"
        @hide="errorMessage = ''"
      ></mp-toptips>
      <div class="create-project-dialog__content">
        <div class="name">
          <h2>创建工程</h2>
        </div>
        <mp-form-page class="formpage">
          <mp-form
            class="rule-form"
            ref="ruleForm"
            :rules="rules"
            :models="form"
          >
            <mp-cells>
              <mp-cell show-error="true" prop="name">
                <div slot="title"><div class="title">工程名称</div></div>
                <wx-input
                  class="wx-input"
                  :value="form.name"
                  data-field="name"
                  placeholder="请输入工程名称"
                  placeholder-class="wx-input-placeholder"
                  @input="onFormInputChange"
                ></wx-input>
              </mp-cell>
            </mp-cells>
          </mp-form>
          <div slot="button">
            <wx-button
              class="common-btn"
              type="primary"
              :disabled="isLoading"
              :loading="isLoading"
              @click="handlerSubmit('ruleForm')"
              >确定</wx-button
            >
          </div>
        </mp-form-page>
      </div>
    </wx-animation>
  </div>
</template>

<script>
export default {
  name: "CreateProject",
  props: {
    visible: {
      type: Boolean,
      require: true
    },
    dataType: {
      type: String,
      required: true
    }
  },
  data() {
    return {
      // 关联类型
      type: {
        achievement: 1,
        demand: 2
      },
      errorMessage: "",
      isLoading: false,
      rules: [
        {
          name: "name",
          rules: { required: true, message: "工程名称不能为空" }
        }
      ],
      form: {
        name: ""
      }
    };
  },
  computed: {
    dockingType() {
      return this.type[this.dataType];
    }
  },
  watch: {
    visible: "showContent"
  },
  methods: {
    // 如果 visible 为 true 则显示
    showContent() {
      this.$nextTick(() => {
        if (this.visible) {
          this.animShowElement(this.$refs.createProject, "animate__fadeIn");
        }
      });
    },
    // 隐藏内容 并将 visible 置为 false
    hideContent() {
      return new Promise((resolve, reject) => {
        this.animHideElement(
          this.$refs.createProject,
          "animate__fadeOut",
          () => {
            this.$emit("update:visible", false);
            resolve();
          }
        );
      });
    },
    // 输入绑定
    onFormInputChange(evt) {
      const { field } = evt.currentTarget.dataset;
      this.form = Object.assign({}, this.form); // 拷贝
      this.form[field] = evt.detail.value;
    },
    // 提交
    handlerSubmit(formName) {
      // 校验
      this.$refs[formName].$$wxCustomComponent.validate((valid, errors) => {
        if (valid) {
          this.loadCreate();
        } else {
          const firstError = Object.keys(errors);
          if (firstError.length) {
            this.errorMessage = errors[firstError[0]].message;
          }
        }
      });
    },
    loadCreate() {
      this.isLoading = true;
      this.$api("project", "createProject", {
        dockingType: this.dockingType,
        ...this.form
      }).then((res) => {
        this.isLoading = false;
        this.$message({
          message: res.message,
          type: res.code === 0 ? "success" : "error"
        });
        if (res.code === 0) {
          this.hideContent().then(() => {
            this.$emit("complete", true);
          });
        }
      });
    }
  }
};
</script>

<style lang="less">
.create-project-dialog {
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  --animate-duration: 400ms;

  .mask {
    position: absolute;
    z-index: 99;
    left: 0;
    top: 0;
    width: 100%;
    height: 100%;
    background-color: rgba(0, 0, 0, 0.3);
  }

  .create-project-dialog__content {
    position: absolute;
    z-index: 100;
    left: 0;
    bottom: 0;
    width: 100%;
    height: 525 / @rem;
    background-color: #ffffff;
    border-radius: 30 / @rem 30 / @rem 0 / @rem 0 / @rem;
    .name {
      padding: 40 / @rem 0;
      text-align: center;
      h2 {
        line-height: 58 / @rem;
        font-size: 40 / @rem;
      }
      p {
        line-height: 44 / @rem;
        color: #666666;
      }
    }
    .formpage {
      .weui-form {
        min-height: auto;
        flex: none;
        .weui-form__control-area {
          flex: none;
          .rule-form {
            .weui-cells {
              margin-top: 0;
              &::before,
              &::after {
                display: none;
              }
              .weui-cell {
                height: 110 / @rem;
                padding: 0;
                font-size: 30 / @rem;
                .wx-input {
                  width: 100%;
                  height: 110 / @rem;
                  line-height: 110 / @rem;
                }
                .wx-input-placeholder {
                  color: #999999;
                }
                .title {
                  padding-right: 30 / @rem;
                }
              }
            }
          }
        }
        .weui-form__opr-area {
          padding-top: 90 / @rem;
        }
      }
    }
  }
}
</style>
