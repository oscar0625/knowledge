<template>
  <div v-if="visible">
    <wx-animation ref="projectApply" class="project-apply-dialog">
      ...
      <mp-toptips
        ...
        @hide="errorMessage = ''"
      ></mp-toptips>
      <div class="project-apply-dialog__content">
        <div class="name">
          <h2>填写申报信息</h2>
        </div>
        <mp-form-page class="formpage">
          ...
          >
            <mp-cells>
              <mp-cell show-error="true" prop="contact">
                <wx-input
                  class="wx-input"
                  :value="form.contact"
                  data-field="contact"
              ...
              <mp-cell show-error="true" prop="post">
                <wx-input
                  class="wx-input"
                  :value="form.post"
                  data-field="post"
...
export default {
  ...
  methods: {
    ...
    showContent() {
      this.$nextTick(() => {
        if (this.visible) {
          this.animShowElement(this.$refs.projectApply, "animate__fadeIn");
        }
      });
    },
    ...
    loadApplyById() {
      this.isLoading = true;
      this.$api("policy", "declareProject", {
        ...this.form,
        itemId: this.dataId
      }).then((res) => {
    ...
    loadApplyByName() {
      this.isLoading = true;
      this.$api("policy", "declareProjectByName", {
        ...this.form,
        ...this.projectDetail
      }).then((res) => {
        this.isLoading = false;
