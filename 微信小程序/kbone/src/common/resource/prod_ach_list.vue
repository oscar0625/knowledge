<template>
  <div class="prod-ach-list">
    <ul>
      ...
      >
        <a :href="`/prodAchInfo?id=${item.id}`" :target="isBlank">
          <div class="title fn-text-ellipsis card-link-icon">
            {{ item.name }}
          </div>
          <div class="detail">
            <div
              class="field"
              :style="`
              background-image: url(${createFieldImg(item.fieldName).a});
            `"
            >
              {{ item.fieldName }}
            </div>
