...
// 默认字体颜色
const fontColor = "#A6BFFF";
// 默认轴线线颜色
const lineColor = "#233F8C";
  ...
  this._option = Object.assign(
    {
      fontColor: fontColor,
      lineColor: lineColor,
      mAxis: "xAxis", //主轴 默认是x轴
      vAxis: "yAxis" //垂直于主轴的轴 默认是y轴
    },
    option
  );
...
Chart.prototype.draw = function() {
  ...
  this._chart = echarts.init(this._option.ele);
...
/**
 * @method
 * @description 重新更新初始化时传入的option的方法 通过此方法可以修改series等(包括data)一切参数
 * @param {object} 和初始化时传入的option一样
 * @param {boolean} 后续是否调用内部_createConfig方法创建config
 * @return {object} 返回调用者本身 支持链式调用
 * @example
 * chart.setData(data).draw();
 */
Chart.prototype.updateOption = function(option, isCreate) {
 ...
 * @param {string} option.unitName 当前图例的单位名称 默认单位
      ...
      legend: Object.assign(
        {
          ...
          padding: [0.25 * basicSize, 0],
          icon: "circle",
          itemGap: 2 * basicSize,
          itemWidth: basicSize,
          itemHeight: basicSize,
          data: option.series.map(function(item, index) {
            var color = option.palletColors[index];
            return {
              name: item.name,
              textStyle: {
          ...
          formatter: function(res) {
            ...
            res.map(function(item, i) {
              ...
              if (Tool.isType(item.color) === "Object") {
                marker =
                  '<span style="background:linear-gradient(to bottom, ' +
        ...
        switch (option.showUnitMethod) {
          case 2:
            res.axisLabel.formatter = function(value) {
              return (
                (curMonad ? value / transUnit + curMonad : value) +
                (option.unit || "")
              );
            };
            break;
          ...
          default:
            ...
            Object.assign(res, {
              name: option.unit ? unitName + "/" + option.unit : "",
              nameGap: 1.5 * basicSize,
              nameTextStyle: {
                padding: [0, 0, 0, (option.unitTextindent || 0) * basicSize]
              }
            });
        }
        return Object.assign(res, option[option.vAxis]);
      })(),
      series: option.series.map(function(item, i) {
        var res = {
          label: {
 ...
 * @return {object} 创建的实例对象
 * @example
 * var chart = new PMBarChart({
      ele: this.$refs.target,
      fontSize: basicSize,
      palletColors: this.colors,
      mAxis: "yAxis",
      vAxis: "xAxis",
      grid: [
        {
      ...
      data: [
        [ { name: "周一", value: 200 }, { name: "周二", value: 170 }, { name: "周三", value: 240 }, { name: "周四", value: 244 }, { name: "周五", value: 200 }, { name: "周六", value: 220 }, { name: "周日", value: 210 } ],
        [ { name: "周一", value: -120 }, { name: "周二", value: -132 }, { name: "周三", value: -101 }, { name: "周四", value: -134 }, { name: "周五", value: -190 }, { name: "周六", value: -230 }, { name: "周日", value: -210 } ]
      ]
    });
chart.draw();
 */
function PMBarChart(option) {
  Chart.call(this, option);
}
_perfectInherit(PMBarChart, Chart, {
  _createConfig: function() {
    var option = this._option,
      basicSize = option.fontSize,
      categoryData = [];
    ...
    this._configuration = {
      ...
      tooltip: Object.assign(
        {
          ...
          formatter: function(res) {
            ...
            res.map(function(item, i) {
              ...
              tem +=
                "<p>" + marker + item.seriesName + "：" + item.value + "</p>";
            });
            tem += "</div>";
            return tem;
          }
      ...
      grid: option.grid.map(function(item) {
      ...
      [option.mAxis]: option[[option.mAxis]].map(function(item, i) {
        return Object.assign(
          {
            gridIndex: i,
            axisLine: {
      ...
      [option.vAxis]: option[[option.vAxis]].map(function(item, i) {
        return Object.assign(
          {
            gridIndex: i,
            axisLine: {
  ...
  _createConfig: function() {
    ...
    this._configuration = {
      ...
      yAxis: (function() {
        ...
        return Object.assign(res, option.yAxis);
      })(),
      series: option.series.map(function(item, i) {
        return Object.assign(
          {
 ...
 * @param {string} option.series.unitName 当前图例的单位名称 默认单位
...
_perfectInherit(BarLineChart, Chart, {
  _createConfig: function() {
    ...
    this._configuration = {
      ...
      yAxis: option.series.map(function(item, index) {
        var monad = {
            1000: "千",
            10000: "万",
            1000000: "百万",
        ...
        switch (item.showUnitMethod) {
          ...
          default:
            ...
            Object.assign(res, {
              name: item.unit ? unitName + "/" + item.unit : "",
              nameGap: 1.5 * basicSize,
              nameTextStyle: {
        ...
        return option.yAxis ? Object.assign(res, option.yAxis[index]) : res;
      }),
      series: option.series.map(function(item, i) {
        var res = {
          yAxisIndex: i,
      ...
      legend: Object.assign(
        {
          ...
          itemWidth: basicSize,
          itemHeight: basicSize,
          data: option.data.map(function(item, index) {
            var color = option.palletColors[index];
            return {
              name: item.name,
              textStyle: {
      ...
      tooltip: Object.assign(
        {
          ...
          formatter: function(res) {
            ...
            if (Tool.isType(res.color) === "Object") {
              marker =
                '<span style="background:linear-gradient(to bottom, ' +
                res.color.colorStops[0].color +
                ", " +
                res.color.colorStops[1].color +
                ');"></span>';
            } else {
              marker = res.marker;
            }
      ...
      series: Object.assign(
        {
          ...
          label: {
            formatter: function(e) {
              var num =
                  option.series.labelDecimal === undefined
                    ? 2
                    : option.series.labelDecimal,
                percent = Tool.keepDecimal(e.percent, num),
...
_perfectInherit(RosePieChart, Chart, {
  _createConfig: function() {
    var option = this._option,
      basicSize = option.fontSize,
      sum = option.data.reduce(function(acc, cur) {
      ...
      legend: Object.assign(
        {
          ...
          itemWidth: basicSize,
          itemHeight: basicSize,
          data: mData.map(function(item, index) {
            var color = option.palletColors[index];
            return {
              name: item.name,
              textStyle: {
            ...
            formatter: function(e) {
              var num =
                  option.series.labelDecimal === undefined
                    ? 2
                    : option.series.labelDecimal,
                percent = Tool.keepDecimal(
                  (e.data.reserveValue / e.data.sum) * 100,
                  num
                ),
                len = option.series.labelNameLength || Infinity,
                name =
 ...
 * @param {string} option.fontColor 图表中字体的颜色 默认#fff
 * @param {array} option.palletColors 图表的调色板，颜色从中依次选取 支持渐变（必填）
 * @param {array} option.indicator 雷达图的坐标系name和max （必填）
      ...
      tooltip: Object.assign(
        {
          ...
          formatter: function(res) {
            var tem =
              ...
              res.seriesName +
              "</p>";
            res.value.map(function(v, i) {
              tem += "<p>" + option.indicator[i].name + "：" + v + "</p>";
            });
            tem += "</div>";
            return tem;
          }
